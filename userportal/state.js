// ShippingCow — Global State + Ingest Pipeline
...
window.SC_STATE = {
  ...
  setProcessedRows(rows, fileMeta) {
    this.processedRows = rows;
    this.hasData = rows.length > 0;
    if (fileMeta) this.siloFiles = [fileMeta, ...this.siloFiles];
    this.notify();
  },
...
// ── Zone lookup (ZIP prefix distance) ────────────────────────────────────────
window.lookupZone = function(originZip, destZip) {
  const o = parseInt((originZip || "00000").slice(0, 3));
  const d = parseInt((destZip  || "00000").slice(0, 3));
...
// ── SC rate factor by zone (discount vs retail) ───────────────────────────────
window.SC_RATE_FACTOR = { 1:0.82, 2:0.80, 3:0.78, 4:0.76, 5:0.74, 6:0.72, 7:0.70, 8:0.68 };

// ── Ingest pipeline ────────────────────────────────────────────────────────────
window.ingestRows = function(mappedRows) {
  return mappedRows.map(row => {
    const l = parseFloat(row.length_in) || 0;
    const w = parseFloat(row.width_in) || 0;
    const h = parseFloat(row.height_in) || 0;
    const actual   = parseFloat(row.actual_weight_lb) || 0;
    const billable = parseFloat(row.billable_weight_lb) || actual;
    const cost     = parseFloat(row.cost_per_package) || 0;
    const pkgs     = parseInt(row.packages_shipped) || 1;
    const zone     = window.lookupZone(row.origin_zip, row.destination_zip);
    const dim_weight = (l * w * h) / 139;
    const dim_overcharge_usd = billable > actual
      ? cost * (billable - actual) / billable : 0;
    const sc_cost = cost * (window.SC_RATE_FACTOR[zone] || 0.75);
    return {
      ...row,
      cost_per_package: cost,
      packages_shipped: pkgs,
      actual_weight_lb: actual,
      billable_weight_lb: billable,
      dim_weight,
      zone,
      dim_overcharge_usd,
      total_row_cost: cost * pkgs,
      sc_cost,
      sc_saving: (cost - sc_cost) * pkgs,
    };
  });
};

// ── Aggregate helpers (used by all surfaces) ──────────────────────────────────
window.SC_AGG = {
  get rows() { return window.SC_STATE.processedRows; },

  // Return rows filtered to a period based on actual ship dates in the data.
  // period: "30d" | "90d" | "6mo" | "12mo" | "custom"
  // customRange: { from: "YYYY-MM-DD", to: "YYYY-MM-DD" } — used only when period === "custom"
  filteredRows(period, customRange) {
    const all = this.rows;
    ...
    const dataMax = new Date(Math.max(...dates));
    ...
    if (period === "custom" && customRange && customRange.from && customRange.to) {
      ...
      cutoff = new Date(dataMax); cutoff.setDate(cutoff.getDate() - 30);
    } else if (period === "90d") {
      cutoff = new Date(dataMax); cutoff.setDate(cutoff.getDate() - 90);
    } else if (period === "6mo") {
      cutoff = new Date(dataMax); cutoff.setMonth(cutoff.getMonth() - 6);
    } else if (period === "12mo") {
      cutoff = new Date(dataMax); cutoff.setFullYear(cutoff.getFullYear() - 1);
    } else {
      return all;
    }
    return all.filter(r => {
      const d = new Date(r.date);
      return !isNaN(d) && d >= cutoff && d <= dataMax;
    });
  },

  // Date range info for display
  dataDateRange() {
    const dates = this.rows.map(r => new Date(r.date)).filter(d => !isNaN(d));
    if (!dates.length) return null;
    return { min: new Date(Math.min(...dates)), max: new Date(Math.max(...dates)) };
  },

  totalSpend(rows) { const r = rows || this.rows; return r.reduce((s, x) => s + x.total_row_cost, 0); },
  totalShipments(rows) { const r = rows || this.rows; return r.reduce((s, x) => s + x.packages_shipped, 0); },

  avgZone(rows) {
    const r = rows || this.rows;
    const tot = this.totalShipments(r);
    if (!tot) return 0;
    return r.reduce((s, x) => s + x.zone * x.packages_shipped, 0) / tot;
  },

  dimOverchargePct(rows) {
    const r = rows || this.rows;
    const n = r.filter(x => x.billable_weight_lb > x.actual_weight_lb).length;
    return r.length ? (n / r.length) * 100 : 0;
  },

  dimOverchargeTotal(rows) { const r = rows || this.rows; return r.reduce((s, x) => s + x.dim_overcharge_usd * x.packages_shipped, 0); },

  annualSavings(rows) {
    const r = rows || this.rows;
    const days = this._dateRangeDays(r) || 90;
    return r.reduce((s, x) => s + x.sc_saving, 0) * (365 / days);
  },

  _dateRangeDays(rows) {
    const r = rows || this.rows;
    const dates = r.map(x => new Date(x.date)).filter(d => !isNaN(d));
    if (dates.length < 2) return 90;
    return Math.max(1, (Math.max(...dates) - Math.min(...dates)) / 86400000);
  },

  zoneDist(rows) {
    const r = rows || this.rows;
    const dist = {};
    for (let z = 1; z <= 8; z++) dist[z] = 0;
    r.forEach(x => { dist[x.zone] = (dist[x.zone] || 0) + x.packages_shipped; });
    return dist;
  },
  ...
  zoneSpend(rows) {
    ...
    const dist = {};
    for (let z = 1; z <= 8; z++) dist[z] = 0;
    r.forEach(x => { dist[x.zone] = (dist[x.zone] || 0) + x.total_row_cost; });
    return dist;
  },
  ...
  pctZone6plus(rows) {
    ...
    const tot = this.totalShipments(r);
    if (!tot) return 0;
    return Object.entries(this.zoneDist(r)).filter(([z]) => z >= 6).reduce((s,[,c]) => s+c, 0) / tot * 100;
  },
  ...
  pctZone4plus(rows) {
    ...
    const tot = this.totalShipments(r);
    if (!tot) return 0;
    return Object.entries(this.zoneDist(r)).filter(([z]) => z >= 4).reduce((s,[,c]) => s+c, 0) / tot * 100;
  },
  ...
  topSkus(n, rows) {
    ...
    const map = {};
    r.forEach(x => {
      if (!map[x.sku]) map[x.sku] = { sku: x.sku, name: x.category || x.sku, ships: 0, totalCost: 0, totalSC: 0 };
      map[x.sku].ships     += x.packages_shipped;
      map[x.sku].totalCost += x.total_row_cost;
      map[x.sku].totalSC   += x.sc_cost * x.packages_shipped;
    });
    return Object.values(map)
      ...
      .map(s => ({
        ...s,
        yourCost: s.totalCost / s.ships,
        scCost:   s.totalSC  / s.ships,
      }));
  },

  carriers(rows) { const r = rows || this.rows; return [...new Set(r.map(x => x.carrier).filter(Boolean))]; },
  platforms(rows) { const r = rows || this.rows; return [...new Set(r.map(x => x.selling_platform).filter(Boolean))]; },
  originZips(rows) { const r = rows || this.rows; return [...new Set(r.map(x => x.origin_zip).filter(Boolean))]; },

  topDestStates(n, rows) {
    n = n || 3;
    const r = rows || this.rows;
    const map = {};
    r.forEach(x => {
      const zip = (x.destination_zip || "").slice(0, 3);
      const state = window.ZIP_TO_STATE?.[zip] || "??";
      if (!map[state]) map[state] = { state, count: 0, cost: 0 };
      map[state].count += x.packages_shipped;
      map[state].cost  += x.total_row_cost;
    });
  ...
  dimInflatePct(rows) {
    const r = rows || this.rows;
    const avg = r.length ? r.reduce((s, x) => s + x.dim_overcharge_usd, 0) / r.length : 0;
    const bench = 1.40;
    return avg > 0 ? Math.round(((avg - bench) / bench) * 100) : 0;
  },

  painPoints(rows) {
    const r = rows || this.rows;
    const days = this._dateRangeDays(r) || 90;
    const ann = v => v * (365 / days);
    const pts = [];
    const dimTotal = ann(this.dimOverchargeTotal(r));
    if (dimTotal > 100) pts.push({ p:"Dim overcharges on bulky SKUs", impact: Math.round(dimTotal), action:"Switch to right-sized boxes for top offenders", sev:"critical" });

    const zd = this.zoneDist(r);
    const zone6plus = (zd[6]||0)+(zd[7]||0)+(zd[8]||0);
    const zoneExcess = ann(zone6plus * 8.94 * 0.28);
    if (zoneExcess > 500) pts.push({ p:"Zone 6+ over-indexing", impact: Math.round(zoneExcess), action:"Add SC node in NJ or CA to cut West Coast zones", sev:"critical" });

    const carrierGap = ann(r.reduce((s,x)=>s+(x.cost_per_package - x.sc_cost)*x.packages_shipped, 0));
    if (carrierGap > 500) pts.push({ p:"Carrier rates above SC negotiated rates", impact: Math.round(carrierGap), action:"Move volume to ShippingCow negotiated FedEx/UPS", sev:"warning" });

    if (pts.length === 0) pts.push({ p:"No critical issues detected", impact:0, action:"Keep monitoring — upload more data for deeper analysis", sev:"opportunity" });
    return pts;
  },

  destStateShipments(rows) {
    const r = rows || this.rows;
    const map = {};
    r.forEach(x => {
      const zip = (x.destination_zip || "").slice(0, 3);
      const abbr = window.ZIP_TO_STATE?.[zip];
      if (abbr) map[abbr] = (map[abbr]||0) + x.packages_shipped;
    });
    return map;
  },
};

// ── ZIP prefix → state abbr (major prefixes) ──────────────────────────────────
window.ZIP_TO_STATE = {
  ...
  "200":"VA","201":"VA","202":"VA","203":"VA","204":"VA","205":"VA","220":"VA","221":"VA","222":"VA","223":"VA","224":"VA","225":"VA","226":"VA","227":"VA","228":"VA","229":"VA","230":"VA","231":"VA","232":"VA","233":"VA","234":"VA","235":"VA","236":"VA","237":"VA","238":"VA","239":"VA","240":"VA","241":"VA","242":"VA","243":"VA","244":"VA","245":"VA","246":"VA",
